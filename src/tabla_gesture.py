import cv2
import math
import time
import mediapipe as mp
from mediapipe.tasks import python
from mediapipe.tasks.python import vision
from enum import Enum

import os
from pathlib import Path

SRC_DIR = Path(__file__).parent.resolve()

MODEL_PATH = str(SRC_DIR.parent / "models" / "hand_landmarker.task")

if not os.path.exists(MODEL_PATH):
    print(f"ERROR: Model file not found at {MODEL_PATH}")
    print("Check if the 'models' folder exists and contains the .task file.")
    exit()

def dist_3d(p1, p2, w, h):
    dx = (p1.x - p2.x) * w
    dy = (p1.y - p2.y) * h
    dz = (p1.z - p2.z) * w
    return math.sqrt(dx**2 + dy**2 + dz**2)  

def main():
    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
        raise RuntimeError("Could not open webcam")

    base_options = python.BaseOptions(model_asset_path=MODEL_PATH)
    options = vision.HandLandmarkerOptions(
        base_options=base_options,
        num_hands=1,
        min_hand_detection_confidence=0.8,
        min_hand_presence_confidence=0.8,
        min_tracking_confidence=0.8,
    )

    landmarker = vision.HandLandmarker.create_from_options(options)

    prev_pinched = [False] * 16
    
    prev_pinched_init_0 = False
    prev_pinched_init_1 = False
    prev_pinched_init_2 = False
    prev_pinched_init_3 = False

    last_trigger_time = [0.0] * 16
    cooldown_s = 0.5

    last_tala_change = 0.0
    TALA_COOLDOWN_S = 0.75   

    class Finger(Enum):
                 MISSING = -1
                 PINKY = 0
                 RING = 1
                 MIDDLE = 2
                 INDEX = 3

    select = Finger.MISSING

    # Talas

    Teentaal = ['Dha', 'Dhin', 'Dhin', 'Dha', 
            'Dha', 'Dhin', 'Dhin', 'Dha',
            'Dha', 'Tin', 'Tin', 'Ta',
            'Ta', 'Dhin', 'Dhin', 'Dha']
    
    Ektaal = ['Dhin', 'Dhin', 'DhaGe', 'Tirkit', 'Tu', 'Na', 'Kat', 'Ta', 'DhaGe', 'Tirkit', 'Dhin', 'Na']
    Ektaal += ['Null'] * 10

    Jhaptaal = ['Dhi', 'Na', 'Dhi', 'Dhi', 'Na', 'Ti', 'Na', 'Dhi', 'Dhi', 'Na']
    Jhaptaal += ['Null'] * 6

    Rupak = ['Ti', 'Ti', 'Na','Dhin', 'Na', 'Dhin', 'Na']
    Rupak += ['Null'] * 9

    AdiTalam = ['Tha', 'Ka', 'Dhi', 'Mi', 'Tha', 'Ka', 'Jhu', 'Nu']
    AdiTalam += ['Null'] * 8

    TalasDict = {"Teentaal (16)": Teentaal, "Ektaal (12)": Ektaal, "Jhaptaal (10)": Jhaptaal, "Rupak (7)": Rupak, "AdiTalam (8)": AdiTalam}
    TalasList = ["Teentaal (16)", "Ektaal (12)", "Jhaptaal (10)", "Rupak (7)", "AdiTalam (8)"]
    TalaIndex = 0
    
    Taal_Chosen = 'Teentaal (16)' # initial
    Taal = TalasDict[Taal_Chosen]
    
    bol = "--"
    while True:
        ret, frame = cap.read()
        if not ret:
            break

        frame = cv2.flip(frame, 1)
        h, w, _ = frame.shape

        mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=frame)
        result = landmarker.detect(mp_image)

        status = "--"
        

        if result.hand_landmarks:
            lm = result.hand_landmarks[0]

            # MediaPipe landmark indices
            
            thumb = lm[4]
            
            class Thumb2:
                def __init__(self, thumb):
                    self.x = thumb.x
                    self.y = thumb.y - 0.02
                    self.z = thumb.z
            
            thumb2 = Thumb2(thumb)


            thumb_xy = (int(thumb.x * w), int(thumb.y * h))
            thumb2_xy = (int((thumb2.x)* w), int((thumb2.y)* h))

            ########## INITIAL #################### 
            pinky3 = lm[20]
            pinky3_xy = (int(pinky3.x * w), int(pinky3.y * h))

            ring3 = lm[16]
            ring3_xy = (int(ring3.x * w), int(ring3.y * h))

            middle3 = lm[12]
            middle3_xy = (int(middle3.x * w), int(middle3.y * h))

            index3 = lm[8]
            index3_xy = (int(index3.x * w), int(index3.y * h))

            wrist = lm[0]
            mid_mcp = lm[9]

            s_dist = dist_3d(wrist, mid_mcp, w, h)   
            PINCH_ON  = s_dist * 0.30
            PINCH_OFF = s_dist * 0.80

            print(s_dist, " pixels") # 115

            rad = int(s_dist / 23)

            cv2.circle(frame, thumb2_xy, rad, (255, 0, 255), -1)

            show_circle = True

            if show_circle & (select == Finger.MISSING):
                cv2.circle(frame, pinky3_xy, rad, (0, 0, 255), -1)
                cv2.circle(frame, ring3_xy, rad, (0, 255, 255), -1)
                cv2.circle(frame, middle3_xy, rad, (0, 255, 0), -1)
                cv2.circle(frame, index3_xy, rad, (255, 0, 0), -1)

            pinch_px_init_pinky = dist_3d(thumb2, pinky3, w, h)
            pinch_px_init_ring = dist_3d(thumb2, ring3, w, h)
            pinch_px_init_middle = dist_3d(thumb2, middle3, w, h)
            pinch_px_init_index = dist_3d(thumb2, index3, w, h)

            pinched_pinky  = pinch_px_init_pinky < 20
            pinched_ring = pinch_px_init_ring < 20
            pinched_middle = pinch_px_init_middle < 20
            pinched_index = pinch_px_init_index < 20
        
            now = time.time()
            if select == Finger.MISSING:
                if (pinched_pinky == True) and (prev_pinched_init_0 == False) and (now - last_trigger_time[3] > cooldown_s):
                    last_trigger_time[3] = now
                    print("Pinky")
                    select = Finger.PINKY
                    status = f"PINCH_PX: {pinch_px_init_pinky:.1f}"

                if pinched_ring and not prev_pinched_init_1 and (now - last_trigger_time[7] > cooldown_s):
                    last_trigger_time[7] = now
                    print("Ring")
                    select = Finger.RING
                    status = f"PINCH_PX: {pinch_px_init_ring:.1f}"
                
                if pinched_middle and not prev_pinched_init_2 and (now - last_trigger_time[10] > cooldown_s):
                    last_trigger_time[10] = now
                    print("Middle")
                    select = Finger.MIDDLE
                    status = f"PINCH_PX: {pinch_px_init_middle:.1f}"

                if pinched_index and not prev_pinched_init_3 and (now - last_trigger_time[13] > cooldown_s):
                    last_trigger_time[13] = now
                    print("Index")
                    select = Finger.INDEX
                    status = f"PINCH_PX: {pinch_px_init_index:.1f}"
                
                pinch_px_reset_check = dist_3d(middle3, index3, w, h)

                taal_iteration_condition = (pinch_px_reset_check < 10) 
                now = time.time()
                if (taal_iteration_condition) and (now - last_tala_change > TALA_COOLDOWN_S):
                    last_tala_change = now
                    TalaIndex += 1
                    if TalaIndex >= len(TalasList): 
                          TalaIndex = 0
                    Taal = TalasDict[TalasList[TalaIndex]] 
                    
            ############### CONDITIONAL #####################################
            pinch_px = []
            offset = 0
            reset_dist_pinky = 100.0
            reset_dist_ring = 100.0
            reset_dist_middle = 100.0
            reset_dist_index = 100.0

            class virtualDigit:
                    def __init__(self, finger):
                        self.x = finger.x
                        self.y = finger.y + 0.05
                        self.z = finger.z

            if select == Finger.PINKY:
                show_circle = False
                
                # pinky landmarks
                pinky0 = lm[17]
                pinky0_virt = virtualDigit(pinky0)
                pinky1 = lm[18]
                pinky2 = lm[19]
                
                # pinky_xy
                pinky0_xy = (int(pinky0.x * w), int(pinky0.y * h))
                pinky1_xy = (int(pinky1.x * w), int(pinky1.y * h))
                pinky2_xy = (int(pinky2.x * w), int(pinky2.y * h))
                
                # pinky circles
                cv2.circle(frame, pinky0_xy, rad, (0, 0, 255), -1)
                cv2.circle(frame, pinky1_xy, rad, (0, 0, 255), -1)
                cv2.circle(frame, pinky2_xy, rad, (0, 0, 255), -1)
                cv2.circle(frame, pinky3_xy, rad, (0, 0, 255), -1)

                #pinky lines
                cv2.line(frame, pinky0_xy, pinky1_xy, (255, 255, 255), 1)
                cv2.line(frame, pinky1_xy, pinky2_xy, (255, 255, 255), 1) 
                cv2.line(frame, pinky2_xy, pinky3_xy, (255, 255, 255), 1) 

                # pinky matras
                pinch_px_matra1 = dist_3d(thumb2, pinky0_virt, w, h)
                pinch_px_matra2 = dist_3d(thumb2, pinky1, w, h)
                pinch_px_matra3 = dist_3d(thumb2, pinky2, w, h)
                pinch_px_matra4 = dist_3d(thumb2, pinky3, w, h)

                pinch_px = [pinch_px_matra1, pinch_px_matra2, pinch_px_matra3, pinch_px_matra4]

                reset_dist_pinky = math.hypot(pinky0_xy[0] - pinky3_xy[0], pinky0_xy[1] - pinky3_xy[1])

                
            elif select == Finger.RING:
                show_circle = False

                # ring landmarks
                ring0 = lm[13]
                ring1 = lm[14]
                ring_virt = virtualDigit(ring0)
                ring0_cxy = (int(((ring0.x + ring1.x) / 2) * w),int(((ring0.y + ring1.y) / 2) * h))
                ring2 = lm[15]

                # ring_xy
                ring0_xy = (int(ring0.x * w), int(ring0.y * h))
                ring1_xy = (int(ring1.x * w), int(ring1.y * h))
                ring2_xy = (int(ring2.x * w), int(ring2.y * h))
                
                # ring circles
                cv2.circle(frame, ring0_xy, rad, (0, 255, 255), -1)
                cv2.circle(frame, ring1_xy, rad, (0, 255, 255), -1)
                cv2.circle(frame, ring2_xy, rad, (0, 255, 255), -1)
                cv2.circle(frame, ring3_xy, rad, (0, 255, 255), -1)

                # ring lines
                cv2.line(frame, ring0_xy, ring1_xy, (255, 255, 255), 1) 
                cv2.line(frame, ring1_xy, ring2_xy, (255, 255, 255), 1) 
                cv2.line(frame, ring2_xy, ring3_xy, (255, 255, 255), 1) 

                # ring matras
                pinch_px_matra5 = dist_3d(thumb2, ring0, w, h)
                pinch_px_matra6 = dist_3d(thumb2, ring1, w, h)
                pinch_px_matra7 = dist_3d(thumb2, ring2, w, h)
                pinch_px_matra8 = dist_3d(thumb2, ring3, w, h)

                pinch_px = [pinch_px_matra5, pinch_px_matra6, pinch_px_matra7, pinch_px_matra8]

                offset = 4

                reset_dist_ring = math.hypot(ring0_xy[0] - ring3_xy[0], ring0_xy[1] - ring3_xy[1])
                
            elif select == Finger.MIDDLE:
                show_circle = False

                # middle landmarks
                middle0 = lm[9]
                middle1 = lm[10]
                middle0_virt = virtualDigit(middle0)
                middle0_cxy = (int(((middle0.x + middle1.x) / 2) * w), int(((middle0.y + middle1.y) / 2) * h))
                middle2 = lm[11]

                # middle_xy
                middle0_xy = (int(middle0.x * w), int(middle0.y * h))
                middle1_xy = (int(middle1.x * w), int(middle1.y * h))
                middle2_xy = (int(middle2.x * w), int(middle2.y * h))

                # middle circles
                cv2.circle(frame, middle0_xy, rad, (0, 255, 0), -1)
                cv2.circle(frame, middle1_xy, rad, (0, 255, 0), -1)
                cv2.circle(frame, middle2_xy, rad, (0, 255, 0), -1)
                cv2.circle(frame, middle3_xy, rad, (0, 255, 0), -1)

                # middle lines
                cv2.line(frame, middle0_xy, middle1_xy, (255, 255, 255), 1) 
                cv2.line(frame, middle1_xy, middle2_xy, (255, 255, 255), 1) 
                cv2.line(frame, middle2_xy, middle3_xy, (255, 255, 255), 1) 

                # middle matras
                pinch_px_matra9 = dist_3d(thumb2, middle0, w, h)
                pinch_px_matra10 = dist_3d(thumb2, middle1, w, h)
                pinch_px_matra11 = dist_3d(thumb2, middle2, w, h)
                pinch_px_matra12 = dist_3d(thumb2, middle3, w, h)

                pinch_px = [pinch_px_matra9, pinch_px_matra10, pinch_px_matra11, pinch_px_matra12]

                offset = 8

                reset_dist_middle = math.hypot(middle0_xy[0] - middle3_xy[0], middle0_xy[1] - middle3_xy[1])
                
            elif select == Finger.INDEX:
                show_circle = False

                #index landmarks
                index0 = lm[5]
                index1 = lm[6]
                index0_virt = virtualDigit(index0)
                index0_cxy = (int(((index0.x + index1.x) / 2) * w), int(((index0.y + index1.y) / 2) * h))
                index2 = lm[7]
                index3 = lm[8]

                # index_xy
                index0_xy = (int(index0.x * w), int(index0.y * h))
                index1_xy = (int(index1.x * w), int(index1.y * h))
                index2_xy = (int(index2.x * w), int(index2.y * h))
                index3_xy = (int(index3.x * w), int(index3.y * h))

                #index circles
                cv2.circle(frame, index0_cxy, rad, (255, 0, 0), -1)
                cv2.circle(frame, index1_xy, rad, (255, 0, 0), -1)
                cv2.circle(frame, index2_xy, rad, (255, 0, 0), -1)
                cv2.circle(frame, index3_xy, rad, (255, 0, 0), -1)    

                # index lines
                cv2.line(frame, index0_xy, index1_xy, (255, 255, 255), 1) 
                cv2.line(frame, index1_xy, index2_xy, (255, 255, 255), 1)
                cv2.line(frame, index2_xy, index3_xy, (255, 255, 255), 1) 

                # index matras
                pinch_px_matra13 = dist_3d(thumb2, index0, w, h)
                pinch_px_matra14 = dist_3d(thumb2, index1, w, h)
                pinch_px_matra15 = dist_3d(thumb2, index2, w, h)
                pinch_px_matra16 = dist_3d(thumb2, index3, w, h)    

                pinch_px = [pinch_px_matra13, pinch_px_matra14, pinch_px_matra15, pinch_px_matra16]

                offset = 12

                reset_dist_index = math.hypot(index0_xy[0] - index3_xy[0], index0_xy[1] - index3_xy[1])

            ############### RESET CONDITION #################
            if ((select != Finger.MISSING) & ( (reset_dist_pinky < 5) | (reset_dist_ring < 5) | (reset_dist_middle < 5) | (reset_dist_index < 5) )):
                 select = Finger.MISSING
                 prev_pinched = [False] * 16
                 print("Reset")
            
            if (select != Finger.MISSING):
                 best = min(pinch_px)
            else:
                 best = 0
            
            for i in range(len(pinch_px)):
                idx = offset + i
                if (prev_pinched[idx] == True):  
                    pinched = (pinch_px[i] < PINCH_OFF) and (pinch_px[i] == best)
                else: 
                     pinched = (pinch_px[i] < PINCH_ON) and (pinch_px[i] == best)

                now = time.time()
                if (pinched == True) and (prev_pinched[idx] == False) and (now - last_trigger_time[idx] > cooldown_s):
                    last_trigger_time[idx] = now
                    cout = str(Taal[idx]) + ' ' + str(idx + 1)
                    if (idx == 0):
                         cout += ' (Sam)'
                    elif (idx == 8):
                         cout += ' Khali'
                    print(cout)
                    bol = f"BOL: {cout}"
                    

                prev_pinched[idx] = pinched
                status = f"PINCH_PX: {pinch_px[i]:.1f}"

        cv2.putText(frame, status, (20, 40),
        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)

        cv2.putText(frame, f"Tala: {TalasList[TalaIndex]}", (20, 80),
        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)

        cv2.putText(frame, bol, (20, 120),
        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
        
        cv2.imshow("TablaGesture", frame)
        if cv2.waitKey(1) & 0xFF == 27:
            break

    landmarker.close()
    cap.release()
    cv2.destroyAllWindows()

if __name__ == "__main__":
    main()
